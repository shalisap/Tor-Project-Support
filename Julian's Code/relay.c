                           ...
                           cell_direction_t cell_direction)
{
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    /* hack_tor: log things out OUT if 'SafeLogging' is off
     */
    if (!get_options()->SafeLogging && circ->purpose != CIRCUIT_PURPOSE_TESTING) {
      log(LOG_INFO,LD_HIST,"RRCO: %s -> %s (%d -> %d)",
          circ->n_conn->_base.address, TO_OR_CIRCUIT(circ)->p_conn->_base.address,
          cell->circ_id, TO_OR_CIRCUIT(circ)->p_circ_id);
    }
    cell->circ_id = circ->n_circ_id; /* switch it */
    or_conn = circ->n_conn;
  } else if (! CIRCUIT_IS_ORIGIN(circ)) {
    /* hack_tor: log things going IN if 'SafeLogging' is off
     */
    if (!get_options()->SafeLogging && circ->purpose != CIRCUIT_PURPOSE_TESTING) {
      log(LOG_INFO,LD_HIST,"RRCI: %s -> %s (%d -> %d)",
          circ->n_conn->_base.address, TO_OR_CIRCUIT(circ)->p_conn->_base.address,
          cell->circ_id, TO_OR_CIRCUIT(circ)->p_circ_id);
    }
